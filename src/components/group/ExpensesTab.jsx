  ...
  return (
    <div className="space-y-4">
      {expenses.map((expense) => {
        ...
        const userSplit = expense.splitWith?.find((s) => s.id === userId);
        ...
        if (!isSettlement && userSplit) {
          if (expense.paidBy === userId) {
            const totalOwedByOthers = expense.splitWith.reduce(
              (acc, s) => (s.id !== userId ? acc + s.share : acc),
              0
            );
            amount = +totalOwedByOthers.toFixed(2);
            status = amount > 0 ? "You lent" : "Self paid";
          } else {
            if (userSplit.share > 0) {
              status = "You owe";
              amount = +userSplit.share.toFixed(2);
            }
          }
        }
